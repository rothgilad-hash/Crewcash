            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 space-y-4">
              ...
              {sortedTeam.length > 0 && activeCatKeys.length > 0 && (
                <div className="space-y-2 pt-2 border-t border-gray-100">
                  <p className="text-xs font-semibold text-gray-500">{isHe ? 'חלוקת קטגוריות:' : 'Category assignments:'} <span className="text-gray-300">({activeCatKeys.length} קטגוריות, {sortedTeam.length} אנשים, {sortedTeam.map(id => catAssignments[id]?.length||0).join('+')} משוייכות)</span></p>
                  {sortedTeam.map(pid => {
                    const p = participants.find(x => x.id === pid)
                    const cats = catAssignments[pid] || []
                    return (
